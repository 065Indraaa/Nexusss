...
// ── HTML Preview ───────────────────────────────────────────
function HtmlPreview({ files, projectType }) {
  const iframeRef = useRef(null);

  if (projectType !== 'html') {
    return (
      <div className="preview-unavailable">
        <div className="preview-unavail-icon">💻</div>
        <div className="preview-unavail-title">Live Preview — HTML projects only</div>
        <div className="preview-unavail-desc">
          This is a <strong>{projectType}</strong> project. To see it:<br /><br />
          1. Go to <strong>Terminal</strong> tab<br />
          2. Run <strong>npm install</strong><br />
          3. Run <strong>npm run dev</strong><br />
  ...
  const combined = useMemo(() => {
    ...
    cssFiles.forEach(cssFile => {
      const tag = `<link rel="stylesheet" href="${cssFile.path}">`;
      html = html.includes(tag)
        ? html.replace(tag, `<style>\n${cssFile.content}\n</style>`)
        : html.replace('</head>', `<style>\n${cssFile.content}\n</style>\n</head>`);
    });
    jsFiles.forEach(jsFile => {
      const tag = `<script src="${jsFile.path}"></script>`;
      html = html.includes(tag)
...
export default function PreviewPanel({ project, activeRole, onUpdateFiles }) {
  ...
  const handleOpenInStackBlitz = () => {
    ...
    addField('project[template]', project.projectType === 'html' ? 'javascript' : 'node');
    ...
    <div className="preview-panel">
      ...
      <div className="preview-body">
        ...
        <div style={{ display: tab === 'preview' ? 'flex' : 'none', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
          <HtmlPreview files={files} projectType={project?.projectType} />
        </div>
